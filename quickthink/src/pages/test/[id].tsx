...
export default function TakeTest() {
  ...
  const test_id = Array.isArray(query.id) ? query.id[0] : query.id;
  const testData = api.tests.getFullTest.useQuery({ test_id: test_id! }).data!;
  const testQna = api.tests.getQuestions.useQuery({ test_id: test_id! }).data!;
  return (
    <>
      <Head>
        <title>{testData?.test?.id}</title>
        <meta name="description" content="Generated by create-t3-app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      {testData && testQna ? (
        <>
          <p>{testData.test?.title}</p>
          {testQna.QnA.map((qna) => {
